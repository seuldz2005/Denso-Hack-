...
* Notes
- We should know how to decide the period to run tSNE and SHAP, i think label
  should be add in late fusion, and SHAP should run when we gonna maintain
  soon... But what about the bin?
- We also should ask how AE-base and Conv1d-AE will affect the GRU as the latent
  space in both of them are different
- Consider using Atention Masking if GRU is not enough ... (more care in
  degradation point) but maybe not need as we use Sliding Window already
