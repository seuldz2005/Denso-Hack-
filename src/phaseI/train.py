...
from data import cut_windows
from model import Conv1DAutoencoder
from loss import reconstruction_loss


def run_training(healthy_windows: np.ndarray, n_sensors: int, window_len: int,
                  latent_dim: int = 16, n_epochs: int = 30, lr: float = 1e-3,
                  batch_size: int = 64, seed: int = 0, device: str = "cpu") -> Conv1DAutoencoder:
    """
    healthy_windows: (n_windows, window_len, n_sensors) -- CHỈ window cắt
    từ vùng khỏe mạnh (xem data.select_healthy_region + cut_windows với
    stride nhỏ). Không đưa window từ vùng suy thoái vào đây.
    ...
    x = torch.from_numpy(healthy_windows).float()
...
def extract_latents_for_engine(model: Conv1DAutoencoder, series: np.ndarray,
    ...
    """
    series: (n_cycles, n_sensors) đã chuẩn hóa (dùng ĐÚNG NormalizationStats
    đã fit lúc train, xem data.py) -- của MỘT engine.
    bin_stride: khoảng cách giữa các bin (ví dụ 10 cycle/bin) -- ĐÂY LÀ
    THAM SỐ QUYẾT ĐỊNH n_bins của engine này, khác hẳn stride nhỏ dùng lúc
    train AE. Xem lại phần phân biệt "window" vs "bin" nếu cần ôn lại.
